...
const Cartpage = () => {
    ...
    useEffect(() => {
        ...
        const value = products.map((dt) => {
            ...
            return <Cartcomponent settotal={settotal} total={total} image={dt.image} prdcts={prdcts} setprdcts={setprdcts} id={dt.id} name={dt.title} description={dt.description} taille={dt.taille} quantity={dt.quantity} price={dt.price} />
        })
        setcartvalue(value)
    }, [products2]);
